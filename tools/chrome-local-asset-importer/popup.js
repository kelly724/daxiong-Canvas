...
const DEFAULT_SERVER = '127.0.0.1:3000';
const LEGACY_LOCAL_SERVER_RE = /^(?:https?:\/\/)?(?:127\.0\.0\.1|localhost):8767\/?$/i;
...
function apiBase(){
  let value = normalizeServerValue(els.server.value);
...
function normalizeServerValue(value){
  ...
  if(LEGACY_LOCAL_SERVER_RE.test(text)) return DEFAULT_SERVER;
  return text;
}
...
async function saveSettings(){
  els.server.value = normalizeServerValue(els.server.value) || DEFAULT_SERVER;
...
function getSettingsPayload(){
  return {
    server: normalizeServerValue(els.server.value) || DEFAULT_SERVER,
...
async function loadSettings(){
  ...
  els.server.value = normalizeServerValue(data.server || (data.port ? `127.0.0.1:${data.port}` : DEFAULT_SERVER)) || DEFAULT_SERVER;
...
async function importSelected(){
  ...
  if(!res.ok) throw new Error(await res.text());
  const data = await res.json();
  const failedItems = (data.items || []).filter(item => !item.ok);
  const failed = failedItems.length;
  const omitted = Number(data.requested || 0) > Number(data.processed || 0)
    ? Number(data.requested || 0) - Number(data.processed || 0)
    : 0;
  const failureTip = failed ? importFailureSummary(failedItems) : '';
  setStatus(`导入完成：成功 ${data.count || 0} 个${failed ? `，失败 ${failed} 个` : ''}${omitted ? `，未处理 ${omitted} 个` : ''}${localFailed ? `，页面读取失败 ${localFailed} 个` : ''}${failureTip}。`);
  els.import.disabled = selected.size === 0;
}

function importFailureSummary(items){
  const errors = [...new Set((items || [])
    .map(item => String(item?.error || '').trim())
    .filter(Boolean))]
    .slice(0, 3);
  return errors.length ? `，原因：${errors.join('；')}` : '';
}
